#include "fanchors.h"
#include "ui_fanchors.h"

fAnchors::fAnchors(QWidget *parent) :
    QWidget(parent),
    m_ui(new Ui::fAnchors)
{
    m_ui->setupUi(this);
...
int fAnchors::pgExec(std::string qSQL, PGconn *conn, PGresult *res)
{
    if (qSQL.empty())
        return -1;
